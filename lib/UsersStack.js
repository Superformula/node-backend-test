...
const UserModel = require('../src/schema/UserModel');
...
class UsersStack extends cdk.Stack {
    constructor(scope, id, props) {
        ...
        // customize 'bad request body' to show more helpful schema validation messages
        new apigateway.CfnGatewayResponse(usersStack, 'apiBadRequestBody', {
        ...
        // add validator for request body
        const bodyValidator = new apigateway.RequestValidator(usersStack, 'bodyValidator', {
            requestValidatorName: `${stackName}-bodyValidator`,
            restApi: api,
        ...
        crudActions.map(entry => {
            ...
            if (crudValidator) {
                // Schema Model for Create and Update
                const userModel = api.addModel(`UserModel${crudName}`, {
                    modelName: `UserModel${crudName}`,
                    description: `Model schema for user ${crudName}`,
                    contentType: 'application/json',
                    schema: UserModel[crudName],
                });

                // Method with validator for Create and Update
                crudResource.addMethod(crudMethod, integration, {
                    requestModels: { 'application/json': userModel },
                    requestValidator: crudValidator,
                });
            } else {
                // Method for Read and Delete
                crudResource.addMethod(crudMethod, integration);
            }
        });
    }
